...
async function findDriveType(id: string) {
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const existing = await findDriveType(params.id);
    if (!existing) {
      return fail("Drive type not found", 404);
    }
    ...
    const nextKey = `drive_type_${nextSlug}`;
    ...
    return ok(mapDriveType(updated, getLocale(request)));
  } catch (error: any) {
    if (error?.code === "P2002") {
      return fail("Drive type key already exists", 409);
    }
    console.error("PUT /api/drive-types/[id] failed", error);
    return fail("Failed to update drive type", 500);
  }
}
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    console.error("DELETE /api/drive-types/[id] failed", error);
    return fail("Failed to delete drive type", 500);
  }
}
